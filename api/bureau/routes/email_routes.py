...
from bureau.services.task_extractor import extract_tasks
from bureau.services.task_store import save_tasks, get_tasks, mark_task_complete, delete_task, get_setting, set_setting, clear_all_tasks, save_feedback, get_feedback_examples, save_task_feedback, get_task_feedback_examples
from bureau.services.ai_service import summarize_email, generate_reply as ai_generate_reply
from bureau.services.gmail_service import (
...
async def chat_with_james_route(req: ChatRequest, request: Request):
    ...
    from bureau.services.ai_service import chat_with_james
    from bureau.services.gmail_service import get_emails
    from bureau.services.task_store import get_tasks
...
def _get_user_email(request: Request):
    ...
    try:
        ...
        with open("task_debug.log", "a") as f:
            import datetime
            f.write(f"{datetime.datetime.now()} - AUTH EMAIL: {email}\n")
            
        if email: return email.lower().strip()
        return None
    except Exception as e:
        with open("task_debug.log", "a") as f:
            f.write(f"{datetime.datetime.now()} - AUTH ERROR: {str(e)}\n")
        return None
...
ANALYSIS_STATUS.clear()  # Force cache clear on cold start
...
async def run_analysis_background(req: AnalyzeRequest, user_email: str, cookie: str, detail_level: str, persona: str):
    ...
    try:
        async with AI_SEMAPHORE:
            ...
            # Hard 8s timeout to survive Vercel Hobby execution window
            try:
                tasks_task = asyncio.wait_for(
                    asyncio.to_thread(extract_tasks, context, task_pos, task_neg, req.retry),
                    timeout=8.0
                )
                summary_task = asyncio.wait_for(
                    asyncio.to_thread(summarize_email, context, detail_level, req.retry, pos_examples, neg_examples, user_email, persona),
                    timeout=8.0
                )
                tasks, summary_data = await asyncio.gather(tasks_task, summary_task)
            except asyncio.TimeoutError:
                logger.error(f"Analysis timed out for {req.id} (Vercel Hard Cap reached)")
                ANALYSIS_STATUS[job_id] = {"status": "error", "message": "Analysis timed out (Vercel Cap)"}
                return
...
async def user_info(request: Request, response: Response):
    try:
        from bureau.services.gmail_service import authenticate_gmail
...
async def analyze_single(req: AnalyzeRequest, request: Request, response: Response, background_tasks: BackgroundTasks):
    ...
    if not user_email:
        from bureau.services.gmail_service import authenticate_gmail, get_user_info
        from googleapiclient.discovery import build
        creds, was_refreshed = authenticate_gmail(cookie)
...
async def consult_coo_on_task(req: dict, request: Request):
    ...
    from bureau.services.task_store import get_tasks
    from bureau.services.ai_service import generate_ai_response, COO_SYSTEM_PROMPT
...
def fetch_tasks(request: Request):
    user_email = _get_user_email(request)
    if not user_email:
        logger.warning("Fetch tasks called without authenticated user email context.")
        return {"tasks": []}
    
    tasks = get_tasks(user_email)
    # DIAGNOSTIC: Log the actual email and count
    print(f"DEBUG: Fetching tasks for '{user_email}'. Count: {len(tasks)}")
...
async def generate_reply_route(req: ReplyRequest, request: Request):
    user_email = _get_user_email(request)
    from bureau.services.ai_service import generate_reply
    from bureau.services.task_store import get_setting
    ...
    # FETCH STYLE MEMORY
    from bureau.services.task_store import get_style_examples
...
async def send_email_route(req: SendEmailRequest, request: Request):
    try:
        ...
        user_email = _get_user_email(request)
        if user_email:
            from bureau.services.task_store import add_style_reference
...
async def extract_tasks_route(req: dict, request: Request):
    async with AI_SEMAPHORE:
        ...
        task_pos, task_neg = ([], [])
        if user_email:
            from bureau.services.task_store import get_task_feedback_examples
...
async def archive_thread_route(thread_id: str, request: Request):
    from bureau.services.gmail_service import archive_thread
...
async def clear_cache():
    ...
    return {"message": "Cache cleared", "count": 0}
