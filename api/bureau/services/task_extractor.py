...
import re
import hashlib
from bureau.services.ai_service import generate_ai_response, MODELS
...
JSON OUTPUT:"""
    ...
    import logging
    _logger = logging.getLogger("mailpilot.extractor")
    _logger.info(f"RAW EXTRACTOR OUTPUT: {raw_response[:500]}")

    
    # Parse JSON from response
    from bureau.services.ai_service import extract_json
    tasks = extract_json(raw_response)
    
    # Safety: if a single task dict came back instead of a list, wrap it
